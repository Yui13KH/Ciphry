...
// XOR cipher function: input_len bytes XORed with key (key_len bytes), output written to output
...
void xor_run(int argc, char **argv) {
    ...
    if (strcmp(mode, "--encrypt") == 0 || strcmp(mode, "encrypt") == 0) {
        ...
        xor_cipher((const unsigned char *)text, input_len, key, key_len, output);
        ...
        if (hex_len % 2 != 0) {
            fprintf(stderr, "Error: Encrypted hex string length must be even.\n");
            return;
        }

        size_t byte_len = hex_len / 2;
        unsigned char *bytes = malloc(byte_len);
        if (!bytes) { perror("malloc"); return; }
        ...
        if (!output) { perror("malloc"); free(bytes); return; }
